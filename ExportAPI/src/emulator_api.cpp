...
void mesen_initialize_emu(const char* home_folder, bool no_audio, bool no_video, bool no_input) {
    static struct {
        char _;
    } fake_window;
    ...
    InitializeEmu(home_folder, &fake_window, &fake_viewer, true, no_audio, no_video, no_input);

    EmulationConfig emulation_config;
    emulation_config.RewindSpeed = 300;
    _emu->GetSettings()->SetEmulationConfig(emulation_config);
}
...
void mesen_take_screenshot(const char* save_to_path) {
    _emu->GetVideoDecoder()->TakeScreenshot(save_to_path);
}

double mesen_get_aspect_ratio() { return GetAspectRatio(); }
...
void mesen_add_known_game_folder(const char* folder) { AddKnownGameFolder((char*)folder); }

void mesen_stop() { Stop(); }
void mesen_pause() { Pause(); }
void mesen_resume() { Resume(); }

void mesen_display_message(const char* title,
                           const char* msg,
                           const char* param1,
                           const char* param2) {
